...
#done
echo "$(awk 'match($0, /[a-zA-Z0-9]{3} +[0-9]{1,2} [0-9]{2} ([a-zA-Z0-9_\-\[\]]+)/, groups) {print groups[1]}' < $1/failed_login_data.txt)" >> $1/tmp_failed_username_data.txt
...
echo "$(awk 'match($0, /[ ]+([0-9]+)[ ]+([a-zA-Z0-9_\-]+)/, groups) {print "data.addRow([\x27" groups[2] "\x27, " groups[1] "]);"}' < $1/tmp_username_amount_data.txt)" >> $1/tmp_data.txt
