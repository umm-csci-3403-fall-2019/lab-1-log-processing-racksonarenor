...
awk 'match($0, /[a-zA-Z0-9]{3} +[0-9]{1,2} ([0-9]{2}) [a-zA-Z0-9_\-\[\]]+/, groups) {print groups[1]}' < "$1"/failed_login_data.txt >> "$1"/tmp_failed_hours_data.txt
...
awk 'match($0, /[ ]+([0-9]+)[ ]+([0-9]+)/, groups) {print "data.addRow([\x27" groups[2] "\x27, " groups[1] "]);"}' < "$1"/tmp_hours_amount_data.txt >> "$1"/tmp_data.txt
